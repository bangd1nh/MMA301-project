...
const CheckoutScreen = () => {
  ...
  const { cart, getTotalItems, clearCart, addOrder } = useCart(); // Thêm addOrder
  ...
  const handleConfirmOrder = () => {
    if (validateForm()) {
      const order = {
        ...
        total: totalAmount,
        shippingInfo: { ...form },
        paymentMethod: form.paymentMethod || "Chưa chọn",
      };

      if (form.paymentMethod === "cod") {
        Alert.alert(
          "Bạn đã chọn thanh toán trực tiếp",
          "Đặt hàng thành công.",
          [
            {
              text: "OK",
        ...
        navigation.navigate("Payment"); // Chuyển đến Payment để thanh toán
        addOrder(order); // Lưu đơn hàng trước khi chuyển (có thể điều chỉnh nếu cần hoàn tất thanh toán trước)
      } else {
    ...
    >
      ...
      >
        <Text style={styles.orderText}>
          Đơn hàng ({getTotalItems()} sản phẩm) {isExpanded ? "▼" : "▲"}
        </Text>
